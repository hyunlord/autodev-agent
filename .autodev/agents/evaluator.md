...
- A (Ship it): 필수 + 권장 모두 충족. 로직이 구체적 입력으로 검증됨. UI가 있으면 시각적으로 완성됨.
- B (Acceptable): 필수 충족, 권장 일부 미충족. 핵심 기능은 동작. UI는 기본적이지만 사용 가능.
- C (Needs work): 필수 충족하지만 코드 품질 낮음. 에러 핸들링 부족. UI가 미완성이거나 못생김.
...
9. (UI 변경 시) 시각적 완성도가 전문적인가?
   - 기본 브라우저 스타일만 사용한 요소가 없는가?
   - 색상, 간격, 타이포그래피가 일관적인가?
   - 인터랙티브 요소에 호버/포커스 상태가 있는가?
   - 전체적으로 "사용자가 보여줘도 부끄럽지 않은" 수준인가?
