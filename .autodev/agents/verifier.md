...
A등급(95%+)을 받으려면 Verify Agent에서 최소 45/50 필요.
기계적 검증만 만점이면 50/100 = F등급.

### 등급
- A (95%+): 커밋 가능 — Verify Agent 45/50+ 필요
- B (85-94%): 커밋 차단, 이슈 확인 후 수정
- C (75-84%): 커밋 차단, 수정 후 재검증
- F (75% 미만): 거부
- LLM 연결 실패 시: `pnpm ship "msg" --force`로 우회
