    ...
    <div className="flex gap-4">
      ...
      <div className="flex-1 pb-6 min-w-0">
        <div className="flex items-center gap-2 mb-1.5 flex-wrap">
          <span className="text-sm font-medium" style={{ color: 'var(--text-primary)' }}>{stageLabels[stage] ?? stage}</span>
          {duration && (
            <span className="text-xs px-2 py-0.5 rounded-full bg-emerald-500/15 text-emerald-400">{duration}</span>
          )}
          {agentId && <span className="text-xs" style={{ color: 'var(--text-secondary)' }}>{agentId}</span>}
        </div>

        {/* Planning 카드 */}
        {stage === 'planning' && planData && (
          <div className="rounded-lg p-3 border" style={{ background: 'var(--bg-card)', borderColor: 'var(--border-color)' }}>
            <p className="text-xs mb-2" style={{ color: 'var(--text-secondary)' }}>{planData.summary}</p>
            <div className="flex gap-2 flex-wrap">
              {planData.estimatedFiles?.slice(0, 5).map(f => (
                <span key={f} className="text-xs px-2 py-0.5 rounded font-mono" style={{ background: 'var(--bg-card-hover)', color: 'var(--text-secondary)' }}>{f}</span>
              ))}
              {planData.estimatedFiles?.length > 5 && (
                <span className="text-xs px-2 py-0.5 rounded" style={{ background: 'var(--bg-card-hover)', color: 'var(--text-secondary)' }}>
        ...
        {stage === 'coding' && (codingSummary || addedLines != null) && (
          <div className="rounded-lg p-3 border" style={{ background: 'var(--bg-card)', borderColor: 'var(--border-color)' }}>
            {codingSummary && <p className="text-xs mb-2" style={{ color: 'var(--text-secondary)' }}>{codingSummary}</p>}
        ...
        {stage === 'verifying' && verifyData && (
          <>
            <div className="grid grid-cols-2 gap-2.5 mb-2.5">
              <div className="rounded-lg p-3 border" style={{ background: 'var(--bg-card)', borderColor: 'var(--border-color)' }}>
                <p className="text-xs mb-1" style={{ color: 'var(--text-secondary)' }}>Mechanical checks</p>
              ...
              {verifyData.score != null && (
                <div className="rounded-lg p-3 border" style={{ background: 'var(--bg-card)', borderColor: 'var(--border-color)' }}>
                  <p className="text-xs mb-1" style={{ color: 'var(--text-secondary)' }}>LLM verdict</p>
            ...
            {verifyData.screenshotUrl && (
              <div className="rounded-lg overflow-hidden border mt-2.5" style={{ background: 'var(--bg-card)', borderColor: 'var(--border-color)' }}>
                <div className="px-3 py-1.5 text-xs border-b" style={{ color: 'var(--text-secondary)', borderColor: 'var(--border-color)' }}>Screenshot</div>
                <img src={verifyData.screenshotUrl} alt="Screenshot" className="w-full max-h-48 object-contain" style={{ background: 'var(--bg-primary)' }} />
              </div>
            )}
          </>
        )}
